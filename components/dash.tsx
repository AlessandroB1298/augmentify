...
export function Dash() {
  ...
  const [aiResponse, setAiResponse] = useState<string>("Nothing for Now");
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-1 ">
          <div className="bg-white rounded-xl shadow-md p-4 h-80">
            ...
            <div className="h-64 ">
              <p className="text-rose-300 ">{aiResponse}</p>
            </div>
          </div>
        </section>
      </div>
...
function StatCard({ name, project_type, description }: userData) {
  return (
    <div className="bg-white p-4 rounded-xl shadow-md transform duration-75 ease-in hover:bg-gray-200 hover:translate-y-1 hover:cursor-pointer">
      ...
      <p className="mt-1 text-sm font-medium text-green-600">{description}</p>
    </div>
  );
}
